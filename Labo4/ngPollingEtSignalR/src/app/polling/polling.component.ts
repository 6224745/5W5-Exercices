...
export class PollingComponent implements OnInit {
  ...
  async complete(id: number) {
    ...
    let x = await lastValueFrom(this.http.get<any>(this.apiUrl + "Complete/" + id));
    console.log(x);
  }

  async addtask() {
    // TODO On invoke la méthode pour ajouter une tâche sur le serveur (Contrôleur d'API)
    let x = await lastValueFrom(this.http.post<any>(this.apiUrl + "Add?taskText="+ this.taskname, null));
    this.tasks.push(x)
    console.log(x);
  }
  ...
  async updateTasks() {
    ...
    let x = await lastValueFrom(this.http.get<any>(this.apiUrl + "GetAll"));
    this.tasks = x;
